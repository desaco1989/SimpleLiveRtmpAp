package com.desaco.livertmpport.av_process_jni;
...
public class MediaPublisher {
    ...
    //LinkedBlockingQueue的put,add跟offer的区别;put take队列可能会发生阻塞
    ...
    private void setListener() {
        mVideoGatherer.setCallback(new VideoGatherer.Callback() {
            ...
            public void onReceive(byte[] data, int colorFormat) {
                if (isPublish) {
                    mMediaEncoder.putVideoData(data);//TODO VideoGatherer视频数据回传，put到MediaEncoder
                }
            }
        });
        ...
        mAudioGatherer.setCallback(new AudioGatherer.Callback() {
            ...
            public void audioData(byte[] data) {
                if (isPublish) {
                    mMediaEncoder.putAudioData(data);//TODO AudioGatherer音频数据回传，put到MediaEncoder
                }
            }
        });

        mMediaEncoder.setCallback(new MediaEncoder.Callback() {//TODO 视频数据回传
            @Override
    ...
    private void onEncodedAvcFrame(ByteBuffer bb, final MediaCodec.BufferInfo vBufferInfo) {
        int offset = 4; //TODO 处理视频数据 视频轨道
        //判断帧的类型
        if (bb.get(2) == 0x01) {//通过get(int  index)方法来获取position到limit之间指定的数据。
            offset = 3;
        }
        ...
        } else if (type == NAL_SLICE || type == NAL_SLICE_IDR) {//NAL_SLICE_IDR,I关键帧
    ...
    private void onEncodeAacFrame(ByteBuffer bb, final MediaCodec.BufferInfo aBufferInfo) {
        //TODO 处理音频数据 ，音频轨道，关键帧
        if (aBufferInfo.size == 2) {
            // 我打印发现，这里应该已经是吧 关键帧计算好了，所以我们直接发送
